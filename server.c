...
#include <pthread.h> /* fire separate*/
...
/* scrie tot textul in descriptor */
...
/* porneste firele unix, inet si ws api si asteapta doar ce a fost creat */
static int ruleazaskeleton(const struct contextserver *context)
{
    ...
    /* sterg socketul vechi inainte de pornirea componentei unix */
...
cleanup:
    /* chiar daca un create esueaza, fac join doar pe firele pornite corect */
    /* NULL inseamnanseamn succes, orice alta valoare inseamna esec */
    rezultat = NULL;
...
int main(int argc, char **argv)
{
    ...
    /* citesc contextul o singura data si il dau tuturor componentelor */
    if (incarca_context(optiuni.caleconfig, &context) != 0) {
        return EXIT_FAILURE;
    }

    /* directorul de output trebuie pregatit inainte de demo si rapoarte */
    ...
    /* fara -d verific doar configuratia si afisez sumarul */
    if (optiuni.moddemo == 0) {
        return EXIT_SUCCESS;
    }

    /* demo-ul real porneste din componenta inet dupa ce sumarul a fost afisat */
...
pentru testarea intregului milestone 1 :


cd /home/vboxuser/Desktop/PCD/Sound-Sequencer

sh ./build.sh

for f in \
  Sounds/violin-wav/violin_A3_025_forte_arco-normal.wav \
  Sounds/violin-wav/violin_A3_025_mezzo-forte_arco-normal.wav \
  Sounds/cello-wav/cello_A2_025_forte_arco-normal.wav \
  Sounds/cello-wav/cello_A2_025_mezzo-piano_arco-normal.wav
do
  test -f "$f" && echo "OK $f" || { echo "LIPSA $f"; exit 1; }
done

./server -c config/sound_sequencer.cfg -d -e
./admin_client -c config/sound_sequencer.cfg -l -e
./ordinary_client -c config/sound_sequencer.cfg -s \
  -i Sounds/violin-wav/violin_A3_025_forte_arco-normal.wav \
  -n demo_string_layers -e
python3 ordinary_client_alt.py --host 127.0.0.1 --port 19090 \
  --composition demo_string_layers --show-request
