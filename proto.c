...
/* scrie tot textul in descriptor, ca in tema 4 */
...
/* scrie un text complet in descriptor, ca in tema 4 ex1 */
static int scrietext(int descriptor, const char *text)
{
    return scrietot(descriptor, text, strlen(text));
}

/* citeste exact numarul cerut de octeti; util la antete si chunk-uri wav */
...
/* adauga text la finalul unui buffer */
...
/* obtine informatii despre utilizator fara getenv, ca sa ramana thread-safe */
...
/* creeaza directorul de output o singura data */
...
/* scrie raportul unui strat procesat de un copil, ca sa ramana clar ce a rulat*/
...
static int scrieaudiofinal(const struct setariserver *setari, const struct strat *straturi, int numarstraturi) /* NOLINT(readability-function-cognitive-complexity) */
{
    ...
    /* citesc mai intai toate fisierele ca sa aflu lungimea maxima a mixului */
    for (int index = 0; index < numarstraturi; index++) {
...
int ruleazademo(const struct contextserver *context)
{
    ...
    /* fiecare strat merge in procesul lui, ca sa pastrez modelul din tema de procese */
